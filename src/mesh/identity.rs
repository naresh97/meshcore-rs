...
impl LocalIdentity {
    ...
    pub fn from_private_key(private_key: &[u8]) -> Option<Self> {
        ...
        a_bytes.copy_from_slice(private_key.get(0..32)?);
        let a = Scalar::from_bytes_mod_order(a_bytes);
        let a: EdwardsPoint = a * ED25519_BASEPOINT_POINT;
        let public = a.compress().to_bytes();
        Some(Self {
            private: private_key.try_into().ok()?,
            public,
        })
    }

    pub fn calculated_shared_key(&self, other: [u8; PUBLIC_KEY_SIZE]) -> [u8; PUBLIC_KEY_SIZE] {
        let mut e = [0u8; 32];
...
mod tests {
    ...
    #[test]
    fn generate_public() {
        let public = "12346BFDBAA49BFBBE6E7A922CABA326294E11D9C72A24A29FBB3DB708FFF1E0";
        let private = "104B70BC64F3FDBDEC6E9A9189C40C7B6A64E5D3A91B75D423EDF879C4C082605F852A0F473307596502D95238CE1FEC32C4BEBD7D119AE73974C2BFA650A1B3";
        let private = hex::decode(private).unwrap();
        let identity = LocalIdentity::from_private_key(&private).unwrap();
        assert_eq!(hex::decode(public).unwrap(), identity.public);
    }
    ...
    fn sign() {
        ...
        let message = format!("{public}{timestamp}{data}");
    ...
    #[test]
    fn shared() {
        let ciphertext = "C29F50EE338DAF98A4BDC60064E95A07C8BC";
        ...
        let shared = identity.calculated_shared_key(other.try_into().unwrap());
